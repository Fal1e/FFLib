...
function PANEL:Init()
	self:SetBackgroundColor(FFLib.Theme.Main)
	self:SetRounded(6)
	self:SetPlaceholder("")
	...
	self.textentry:SetFont(FFLib.Fonts:Get('default', 20, false))
	...
	self.textentry.OnMousePressed = function(pnl, key)
...
function PANEL:DrawBackground(w, h)
    ...
    if skin and skin.TextEntry and skin.TextEntry.Background then
        skin.TextEntry.Background(self, w, h)
    end
end
...
FFLib.VGUI:Register('TextEntry', PANEL, nil)