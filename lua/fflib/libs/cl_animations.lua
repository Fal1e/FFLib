...
function PNL:LerpColor(var, to, duration, callback)
	if (!duration) then duration = FFLib.TransitionTime end

	local color = self[var]
	local anim = self:NewAnimation(duration)
	anim.Color = to
	anim.Think = function(anim, pnl, fract)
		local newFract = FFLib:Ease(fract, 0, 1, 1)

		if (!anim.StartColor) then
			anim.StartColor = color
		end

		local newColor = FFLib:LerpColor(newFract, anim.StartColor, anim.Color)
		self[var] = newColor
	end
	anim.OnEnd = function()
		if callback then
...
function PNL:LerpVector(var, to, duration, callback)
	...
	anim.Think = function(anim, pnl, fract)
		...
		local newColor = FFLib:LerpVector(newFract, anim.StartVector, anim.Vector)
		self[var] = newColor
	end
	anim.OnEnd = function()
		if callback then
...
function PNL:LerpAngle(var, to, duration, callback)
	...
	anim.Think = function(anim, pnl, fract)
		...
		local newColor = FFLib:LerpAngle(newFract, anim.StartAngle, anim.Angle)
		self[var] = newColor
	end
	anim.OnEnd = function()
		if callback then
...
function PNL:LerpHeight(height, duration, callback, easeFunc)
	...
	if (!easeFunc) then easeFunc = function(a, b, c, d)
			return FFLib:Ease(a, b, c, d)end end
